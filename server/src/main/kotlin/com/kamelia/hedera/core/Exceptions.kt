...
class InvalidUUIDException :
    HederaException(Errors.Parsing.INVALID_UUID)

class InvalidCredentialsException :
    HederaException(Errors.Auth.INVALID_CREDENTIALS)

class AccountDisabledException :
    HederaException(Errors.Auth.ACCOUNT_DISABLED)
...
class MissingTokenException :
    HederaException(Errors.Tokens.MISSING_TOKEN)

class ExpiredOrInvalidTokenException :
    HederaException(Errors.Tokens.EXPIRED_OR_INVALID)

class UploadCodeGenerationException :
    HederaException(Errors.Uploads.TOKEN_GENERATION)

class MultipartParseException :
    HederaException(Errors.Uploads.MULTIPART_PARSE)
...
class InsufficientPermissionsException :
    HederaException(Errors.INSUFFICIENT_PERMISSIONS)
...
class FileNotFoundException :
    HederaException(Errors.Files.NOT_FOUND)

class UserNotFoundException :
    HederaException(Errors.Users.NOT_FOUND)

class PersonalTokenNotFoundException :
    HederaException(Errors.PersonalTokens.NOT_FOUND)

class DisabledRegistrationsException :
    HederaException(Errors.Registrations.DISABLED)

class ThumbnailGenerationException :
    HederaException(Errors.Thumbnails.GENERATION)

class MailSendingException :
    HederaException(Errors.MAIL_NOT_SENT)

class PasswordResetMessagingException :
    HederaException(Errors.Users.ResetPassword.REQUEST_NOT_SENT)

class TooManyPasswordResetRequestsException :
    HederaException(Errors.Users.ResetPassword.TOO_MANY_REQUESTS)

class InvalidPersonalTokenException(locale: Locale = Locale.en) :
    HederaException(
        MessageKeyDTO(
