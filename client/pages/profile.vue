...
<template>
  <div class="flex flex-col">
    <div class="flex flex-col sticky top-0 bg-[var(--ground)] z-100">
      <div class="adaptive-width flex overflow-y-auto self-center py-4 px-4 sm:px-8 w-full xl:w-70%">
        <TabNavigation :items="menus" :route-match="(path, item) => path.endsWith(item.path)" />
      </div>
      <hr class="sep">
    </div>
    <div class="adaptive-width self-center px-4 sm:px-8 my-5 w-full xl:w-70%">
      <NuxtPage />
    </div>
  </div>
</template>
